...
def main():
    ...
    logging.basicConfig(
        filename="simu-graphmodel-sem_type.log",
    ...
    for n in [1000]:
        for graph_type in ["ER", "SF"]:
            ...
            filenameW = os.path.join(default_save_path, f"W_true_n{n}_{graph_type}.csv")
            ...
            # Choose to either generate a new W_true or use the existing W_true from the CSV file
            # W_true = simulate_parameter(B_true)
            # np.savetxt(filenameW, W_true, delimiter=",")
            W_true = pd.read_csv(filenameW, header=None)  # Use the existing one
            W_true = W_true.values
            random.seed(42)
            print(f"graph_type={graph_type}")
            logging.info(f"graph_type={graph_type}")
