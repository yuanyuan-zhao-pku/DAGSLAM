...
# Loop for different d 
for (n in c(1000)) {   
  for (d in c(10, 20, 40, 100)) {   
    print(paste0("d=",d))  
    library(tidyverse)  
    start_time <- Sys.time()  
...
write.csv(results_avg, file.path(default_save_path, "acc_d_hc.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_d_hc.csv"), row.names = FALSE)   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_d_tabu.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_d_tabu.csv"), row.names = FALSE)   

# mDAG -------------------------------------------------------------  
library(mDAG)  

# Define loss type ('g' for 'gauss' and 'c' for 'categorical')
get_loss_type <- function(d) {  
  if (d == 10) {  
    return(c("g", "g", "g", "g", "g", "g", "g", "g", "c", "g"))  
  } else if (d == 20) {  
    return(c(rep("g", 18), "c", "g"))  
  } else if (d == 40) {  
    return(c(rep("g", 29), "c", rep("g", 10)))  
  } else if (d == 100){
    return(c(rep("g", 70), "c", rep("g", 29)))
  }else {  
    stop(paste("Invalid value for d:", d))  
  }  
}

# Define levels for categorical nodes
get_level <- function(d) {  
  loss_type_str <- get_loss_type(d)  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_d_mDAG.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_d_mDAG.csv"), row.names = FALSE)   
...
get_loss_type <- function(d) {  
  if (d == 10) {  
    return(c(rep("c", 8), "b", "c"))  
  } else if (d == 20) {  
    return(c(rep("c", 18), "b", "c"))  
  } else if (d == 40) {  
    return(c(rep("c", 29), "b", rep("c", 10)))  
  } else if (d==100){
    return(c(rep("c", 70), "b", rep("c", 29)))  
  }else {  
    stop(paste("Invalid value for d:", d))  
  }  
}  
...
for (n in c(1000)) {  
  for (d in c(10, 20, 40, 100)) {
    ...
    for (i in 0:9) {  
      ...
      W_est <- dagbagMtoW(X, nodeType = get_loss_type(d))  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_d_DAGBagM.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_d_DAGBagM.csv"), row.names = FALSE)