...
write.csv(results_avg, file.path(default_save_path, "acc_m_HC.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_m_HC.csv"), row.names = FALSE)   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_m_TABU.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_m_TABU.csv"), row.names = FALSE)   
...
get_loss_type <- function(m) {
  return(c(rep("g", 20)))
}
# Define levels for categorical nodes
get_level <- function(m) {  
  loss_type_str <- get_loss_type(m)  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_m_mDAG.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_m_mDAG.csv"), row.names = FALSE)   
...
for (n in c(1000)) {  
  for (m in c(3, 4)) {   
    ...
    for (i in 0:9) {  
      ...
      W_est <- dagbagMtoW(X, nodeType = get_loss_type(m))  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_m_DAGBagM.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_m_DAGBagM.csv"), row.names = FALSE)