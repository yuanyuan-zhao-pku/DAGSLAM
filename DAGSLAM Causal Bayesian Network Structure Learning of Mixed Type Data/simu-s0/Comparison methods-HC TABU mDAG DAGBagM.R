...
# Loop for different s0 
for (n in c(1000)) {  
  for (s0 in c(10, 20, 40)) {   
    print(paste0("s0=",s0))  
    library(tidyverse)  
    start_time <- Sys.time()  
    ...
    filenameB <- file.path(default_save_path, sprintf("B_true_n%d_s0%d.csv", n, s0))
    B_true <- read.csv(filenameB, header = FALSE) %>% as.matrix()  
    
    for (i in 0:9) {  
      print(paste0("i=", i))  
      
      filenameWE <- file.path(default_save_path, sprintf("W_est_n%d_s0%d_i%d.csv", n, s0, i))  
      filenameX <- file.path(default_save_path, sprintf("X_n%d_s0%d_i%d.csv", n, s0, i))        
      ...
      cat(sprintf("n=%d,s0=%d,acc=%s\n", n, s0, paste(ACC[i+1, ], collapse = ",")))  
      cat(sprintf("n=%d,s0=%d,acc_und=%s\n", n, s0, paste(ACC_und[i+1, ], collapse = ",")))  
    }  
    
    acc <- colMeans(ACC)  
    acc_und <- colMeans(ACC_und)  
    cat(sprintf("n=%d,s0=%d,acc=%s\n", n, s0, paste(acc, collapse = ",")))  
    cat(sprintf("n=%d,s0=%d,acc_und=%s\n", n, s0, paste(acc_und, collapse = ",")))  
    
    end_time <- Sys.time()  
    exe_time <- as.numeric(end_time - start_time, units = "secs")  
    cat(sprintf("s0=%d execution time: %.2f seconds\n", s0, exe_time))
...
write.csv(results_avg, file.path(default_save_path, "acc_s0_HC.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_s0_HC.csv"), row.names = FALSE)   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_s0_TABU.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_s0_TABU.csv"), row.names = FALSE)   

# mDAG -------------------------------------------------------------  
library(mDAG)  

# Define loss type ('g' for 'gauss' and 'c' for 'categorical')
get_loss_type <- function(s0) {  
  return(c(rep("g", 18), "c", "g"))  
}  

# Define levels for categorical nodes
get_level <- function(s0) {  
  loss_type_str <- get_loss_type(s0)  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_s0_mDAG.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_s0_mDAG.csv"), row.names = FALSE)   
...
for (n in c(1000)) {  
  for (s0 in c(10, 20, 40)) {   
    ...
    for (i in 0:9) {  
      ...
      W_est <- dagbagMtoW(X, nodeType = get_loss_type(s0))  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_s0_DAGBagM.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_s0_DAGBagM.csv"), row.names = FALSE)