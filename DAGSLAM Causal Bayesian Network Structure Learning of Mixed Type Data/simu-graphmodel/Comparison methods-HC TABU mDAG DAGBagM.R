...
# Loop for different graphmodel 
for (n in c(1000)) {  
  for (graphmodel in c("ER", "SF")) {   
    print(paste0("graphmodel=",graphmodel))  
    library(tidyverse)  
    start_time <- Sys.time()  
    ACC <- matrix(0, nrow = 10, ncol = 6)  
    ACC_und <- matrix(0, nrow = 10, ncol = 6)  
    
    filenameB <- file.path(default_save_path, sprintf("B_true_n%d_%s.csv", n, graphmodel))
    B_true <- read.csv(filenameB, header = FALSE) %>% as.matrix()  
    
    for (i in 0:9) {  
      print(paste0("i=", i))  
      
      filenameWE <- file.path(default_save_path, sprintf("W_est_n%d_%s_i%d.csv", n, graphmodel, i))  
      filenameX <- file.path(default_save_path, sprintf("X_n%d_%s_i%d.csv", n, graphmodel, i))        
      ...
      cat(sprintf("n=%d,graphmodel=%s,acc=%s\n", n, graphmodel, paste(ACC[i+1, ], collapse = ",")))  
      cat(sprintf("n=%d,graphmodel=%s,acc_und=%s\n", n, graphmodel, paste(ACC_und[i+1, ], collapse = ",")))  
    }  
    
    acc <- colMeans(ACC)  
    acc_und <- colMeans(ACC_und)  
    cat(sprintf("n=%d,graphmodel=%s,acc=%s\n", n, graphmodel, paste(acc, collapse = ",")))  
    cat(sprintf("n=%d,graphmodel=%s,acc_und=%s\n", n, graphmodel, paste(acc_und, collapse = ",")))  
    
    end_time <- Sys.time()  
    exe_time <- as.numeric(end_time - start_time, units = "secs")  
    cat(sprintf("graphmodel=%s execution time: %.2f seconds\n", graphmodel, exe_time))
...
write.csv(results_avg, file.path(default_save_path, "acc_graphmodel_HC.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_graphmodel_HC.csv"), row.names = FALSE)   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_graphmodel_TABU.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_graphmodel_TABU.csv"), row.names = FALSE)   

# mDAG -------------------------------------------------------------  
library(mDAG)  

# Define loss type ('g' for 'gauss' and 'c' for 'categorical')
get_loss_type <- function(graphmodel) {  
  return(c(rep("g", 18), "c", "g"))  
}  

# Define levels for categorical nodes
get_level <- function(graphmodel) {  
  loss_type_str <- get_loss_type(graphmodel)  
  ...
  for (graphmodel in c("ER", "SF")) {   
    ...
    for (i in 0:9) {  
      ...
      W_est <- mDAGtoW(data = X,   
                       type = get_loss_type(graphmodel),   
                       level = get_level(graphmodel))   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_graphmodel_mDAG.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_graphmodel_mDAG.csv"), row.names = FALSE)   
...
for (n in c(1000)) {  
  for (graphmodel in c("ER", "SF")) {   
    ...
    filenameB <- file.path(default_save_path, sprintf("B_true_n%d_graphmodel%d.csv", n, graphmodel))
    ...
    for (i in 0:9) {  
      ...
      W_est <- dagbagMtoW(X, nodeType = get_loss_type(graphmodel))  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_graphmodel_DAGBagM.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_graphmodel_DAGBagM.csv"), row.names = FALSE)