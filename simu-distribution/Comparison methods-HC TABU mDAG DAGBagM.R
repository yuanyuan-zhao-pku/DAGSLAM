...
# Loop for different distribution 
for (n in c(1000)) {  
  for (distribution in c("gauss", "exp", "gumbel")) {   
    ...
    filenameB <- file.path(default_save_path, sprintf("B_true_n%d_%s.csv", n, distribution))
    B_true <- read.csv(filenameB, header = FALSE) %>% as.matrix()  
    
    for (i in 0:9) {  
      print(paste0("i=", i))  
      
      filenameWE <- file.path(default_save_path, sprintf("W_est_n%d_%s_i%d.csv", n, distribution, i))  
      filenameX <- file.path(default_save_path, sprintf("X_n%d_%s_i%d.csv", n, distribution, i))        
      ...
      cat(sprintf("n=%d,distribution=%s,acc=%s\n", n, distribution, paste(ACC[i+1, ], collapse = ",")))  
      cat(sprintf("n=%d,distribution=%s,acc_und=%s\n", n, distribution, paste(ACC_und[i+1, ], collapse = ",")))  
    }  
    
    acc <- colMeans(ACC)  
    acc_und <- colMeans(ACC_und)  
    cat(sprintf("n=%d,distribution=%s,acc=%s\n", n, distribution, paste(acc, collapse = ",")))  
    cat(sprintf("n=%d,distribution=%s,acc_und=%s\n", n, distribution, paste(acc_und, collapse = ",")))  
    
    end_time <- Sys.time()  
    exe_time <- as.numeric(end_time - start_time, units = "secs")  
    cat(sprintf("distribution=%s execution time: %.2f seconds\n", distribution, exe_time))
...
write.csv(results_avg, file.path(default_save_path, "acc_distribution_HC.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_distribution_HC.csv"), row.names = FALSE)   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_distribution_TABU.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_distribution_TABU.csv"), row.names = FALSE)   

# mDAG -------------------------------------------------------------  
library(mDAG)  

# Define loss type ('g' for 'gauss' and 'c' for 'categorical')
get_loss_type <- function(distribution) {  
  return(c(rep("g", 18), "c", "g"))  
}  

# Define levels for categorical nodes
get_level <- function(distribution) {  
  loss_type_str <- get_loss_type(distribution)  
  ...
  for (distribution in c("gauss", "exp", "gumbel")) {   
    ...
    for (i in 0:9) {  
      ...
      W_est <- mDAGtoW(data = X,   
                       type = get_loss_type(distribution),   
                       level = get_level(distribution))   
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_distribution_mDAG.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_distribution_mDAG.csv"), row.names = FALSE)   
...
for (n in c(1000)) {  
  for (distribution in c("gauss", "exp", "gumbel")) {   
    ...
    filenameB <- file.path(default_save_path, sprintf("B_true_n%d_distribution%d.csv", n, distribution))
    ...
    for (i in 0:9) {  
      ...
      W_est <- dagbagMtoW(X, nodeType = get_loss_type(distribution))  
...
# Write results to CSV files  
write.csv(results_avg, file.path(default_save_path, "acc_distribution_DAGBagM.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_distribution_DAGBagM.csv"), row.names = FALSE)